 ...
 *
 * Release Version: 23.06.20.17.10 (Jun 20 2023 @ 17:10)
 *
...
=============================================================================
 ...
 $Header: //components/rel/core.boot/6.5.r208.1/QcomPkg/Tools/storage/fh_loader/fh_loader/fh_loader.c#1 $
 $DateTime: 2024/01/30 12:02:09 $
 $Author: mplcsds1 $

when        who  what, where, why
----------  ---  ---------------------------------------------------------
2023-06-15  wek  Include extra dummy parameter in print to fix warnings.
2022-01-26  pz   New arg to improve verify programming with hash.
2021-07-07  wek  Fix error check for write port on linux.
...
To build in Linux:
gcc fh_loader.c fh_loader_sha.c fh_cobs.c fh_crc.c fh_hsuart_packet.c fh_transfer.c fh_transport.c fh_transport_com.c fh_transport_linux_pipe.c fh_transport_hsuart.c stringl/memscpy.c stringl/memsmove.c stringl/strlcat.c stringl/strlcpy.c -o fh_loader -lrt -I stringl/
...
char *CommandLineOptions[] = {  "benchmarkdigestperformance", "benchmarkreads", "benchmarkwrites",
                                "createvipdigests=", "chaineddigests=",
                                "contentsxml=", "convertprogram2read", "digestsperfilename=",
                                "erase=",
                                "files=", "firmwarewrite", "fixgpt=", "flattenbuildto=", "flavor=", "forcecontentsxmlpaths",
                                "getstorageinfo=",
                                "json_in=", "labels=", "loglevel=", "lun=",
                                "mainoutputdir=", "maxpayloadsizeinbytes=", "memoryname=",
                                "notfiles=", "notlabels=", "nop", "noprompt", "num_sectors=",
                                "port=", "porttracename=", "port_type=", "power=",
                                "search_path=", "sectorsizeinbytes=", "sendimage=", "sendxml=", "setactivepartition=",
                                "showpercentagecomplete", "signeddigests=",
                                "slot=", "start_sector=",
                                "verbose", "verify_programming_getsha", "verify_programming", "generatesha256file",
                                "verifysha256file",
                                "zlpawarehost="
                             };

long long unsigned COPY_BUF_SIZE = (4ULL * 1024 * 1024 * 1024) - 1; //4gb max copy buf size
...
char FullMainOutputDirPath[MAX_PATH_SIZE]={0};
...
void time_throughput_calculate (struct timeval *start_time, struct timeval *end_time, uint64 size_bytes, double NetworkElapsed, enum fh_log_lvl ThisLogType);
...
int GetFullDirPath(char *FileFullPath, int MaxLength, char *OutputDirPath);
...
void LoadPowerOpIntoStringTable (enum fh_reboot_options option, unsigned int seconds);
...
*/
void read_chunk_header (FILE * fd, uint16_t chunk_hdr_sz, struct chunk_header_t *chunk_header);
...
struct Attributes_Struct
{
    ...
    char    Raw[4096];    // On PC this handles log values i.e. "8192"
...
struct Attributes_Struct AllAttributes[] =
{
    ...
    {"slot",                      "", (SIZE_T *)&fh.attrs.slot,                          0, 0, 1, 0, NULL, 'i', 0 },
    ...
    */
    ...
    {"sha256",                        "", NULL,                         0, 0, 1,    sizeof (fh.attrs.sha256_verify), (char *) fh.attrs.sha256_verify, 's', 0 }, // 54
};
...
void print_usage (FILE *stream)
{
  char *json_usage = "\nARGUMENTS\n--json_in=  A text file or json formatted file with the following format\n"
    ...
    fprintf (stream, "\nValidation\n"
             "\n\t--generatesha256file. Generate the sha256 file per sendimage by simulation, --digestsperfilename specify the file path."
             "\n\t--verifysha256file.   After flashing image to target, compute sha256 on target side and compare with --digestsperfilename."
      );
    fprintf (stream, "\nImage Validation Usage: \n"
             "fh_loader.exe --port=\\\\.\\COM19 --memoryname=ufs --setactivepartition=1 --generatesha256file --digestsperfilename=C:\\workspace\\fh_loader\\Testsha256file  --json_in=Waipio.LA.1.0-00711-GKI.INT-1.json\n"
             "fh_loader.exe --port=\\\\.\\COM19 --memoryname=ufs --setactivepartition=1 --verify_programming_getsha --verifysha256file --digestsperfilename=C:\\workspace\\fh_loader\\Testsha256file  --json_in=Waipio.LA.1.0-00711-GKI.INT-1.json\n"
             "fh_loader.exe --port=\\\\.\\COM19 --memoryname=ufs --setactivepartition=1 --verify_programming --verifysha256file --digestsperfilename=C:\\workspace\\fh_loader\\Testsha256file  --json_in=Waipio.LA.1.0-00711-GKI.INT-1.json\n"
      );
    fprintf(stream, "%s", json_usage);
...
enum fh_transport_type transport_type = FH_TRANSPORT_NONE;
...
int add_string_to_xml_file_table(char *buffer)
{
  size_t len = strlen (buffer);

  if (CopyString (XMLFileTable[num_xml_files_to_send], buffer, 0, 0, len, MAX_PATH_SIZE, len) == 0)
  {
    dbg (LOG_ERROR, "Failed to copy string '%s' of length %i into XMLFileTable[%i]", buffer, len, num_xml_files_to_send);
    ExitAndShowLog (1);
  }
  ...
  return num_xml_files_to_send;
}

/*
  Logic to keep track of the files that need to be flashed.
*/
struct file_path
{
  char *path; /* Full Path */
...
struct file_list *file_path_init(void)
{
  ...
  if (list == NULL)
  {
    FH_LOGE("Failed to allocate structure to store list of files %d", 0);
    return NULL;
  }
...
static int file_path_grow(struct file_list *list)
{
  if (list->list == NULL)
  {
    size_t i;
    ...
    if (list->list == NULL)
    {
      FH_LOGE("Failed to allocate first buffer for list of files %d", 0);
      return -1;
    }
    list->count = 16;
    list->index = 0;
  ...
  else if(list->count == list->index)   /* If the array is full, allocate more space. */
  {
    ...
    {
      FH_LOGE("Hit the limit of SIZE_T! %llu * %d", (unsigned long long)new_size,
    ...
    if (new_list == NULL)
    {
      FH_LOGE("Failed to grow buffer for list of files %llu", (unsigned long long)new_size);
      return -1;
    }
    list->list = new_list;
    list->count = new_size;
...
int file_path_add_path(struct file_list *list, char *path)
{
  int result = 0;
  char *existing_path;
  if (list == NULL)
  ...
  if (result == 0)
  {
    ...
    {
      FH_LOGE("Supplied path length is too long. Len: %llu path: %s",
    ...
    if (len == 0)
    {
      FH_LOGE("Invalid path length of zero (%llu)", (unsigned long long)len);
      result = -1;
      goto cleanup;
    }
    ...
    if (out_path == NULL)
    {
      FH_LOGE("Failed to allocate buffer for path %s, %llu", path,
    ...
    if (existing_path != NULL)
    {
      ...
      {
        FH_LOGE("Input data/paths have multiple paths for file %s", name);
        ExitAndShowLog (1);
        return -1;
      }
      else
      {
        FH_LOGI("Input points to two %s, but both have the same path", name);
      }
    }
...
int file_path_count(struct file_list *list)
{
  if (list == NULL)
    return 0;
...
int file_path_destroy(struct file_list *list)
{
  size_t i, count;
...
void MyLog (int log_level, const char *func_name, int line_number, const char *format, ...)
{
    ...
    case LOG_DEBUG:
      //snprintf(fileloglevel, sizeof(fileloglevel),"%s DEBUG: %s:%d ", timelog, func_name, line_number);
      snprintf (fileloglevel, sizeof(fileloglevel), "%s DEBUG: ", timelog);
      snprintf (screenloglevel, sizeof(screenloglevel), "%s DEBUG: ", timelog);
        break;
    ...
    case LOG_WARN:
        ...
        {
            printf ("\n\nERROR: Failed to copy string of length %zu bytes into WarningsBuffer", strlen ("\n") );
            ExitAndShowLog (1);
        }

        if ( CopyString (WarningsBuffer, log, strlen (WarningsBuffer), 0, strlen (log), sizeof (WarningsBuffer), sizeof (log) )  == 0)
        {
            printf ("\n\nERROR: Failed to copy string of length %zu bytes into WarningsBuffer", strlen (log) );
            ExitAndShowLog (1);
        }
...
{
  char *log_path = NULL;
  int abs_path_exist = 0; /*to track if absolute_port_trace__name exist or not
                            0 means does not exist, 1 means exist */

  /* at the end we always copy port_trace.txt to cwd if MainOutputDir is cwd/readback
     and delete port_trace.txt from cwd/readback directory
     if MainOutputDir is other than cwd/readback, port_trace.txt is not copied to cwd
     below variable will have the absolute path of cwd/port_trace.txt */
  char final_port_trace_name[MAX_PATH_SIZE]={0};

  /* below variable will have absolute path of port_trace.txt currently
     being used to dump data */
  char absolute_port_trace_name[MAX_PATH_SIZE]={0};

  ClosePort();

  if(global_file_path_list != NULL)
  ...
  else
  {
    ...
    {
      snprintf(absolute_port_trace_name, MAX_PATH_SIZE, "%s"SLASHSTR"%s", MainOutputDir, PortTraceName);
    }
    else
    {
      snprintf(absolute_port_trace_name, MAX_PATH_SIZE, "%s", PortTraceName);
    }
    printf ("\n\nWriting log, might take a minute\n");
    fclose (fp);
    fp = NULL;

    snprintf(final_port_trace_name, MAX_PATH_SIZE, "%s"SLASHSTR"%s", cwd, PortTraceName);
    fp = fopen (absolute_port_trace_name, "rb");
    if(fp)
    {
      /* file at given path exist, so log can be written here */
      fclose(fp);
      fp = NULL;
      abs_path_exist = 1;
    }
  ...
  {
    int result;
    char temp_buffer_1[MAX_PATH_SIZE]={0};
    char temp_buffer_2[MAX_PATH_SIZE]={0};

    result = GetFullDirPath(temp_buffer_1, MAX_PATH_SIZE, absolute_port_trace_name);
    if (result == 0)
      strlcpy(absolute_port_trace_name, temp_buffer_1, sizeof(absolute_port_trace_name));

    result = GetFullDirPath(temp_buffer_2, MAX_PATH_SIZE, final_port_trace_name);
    if (result == 0)
      strlcpy(final_port_trace_name, temp_buffer_2, sizeof(final_port_trace_name));

    memset(temp_buffer_1, 0, MAX_PATH_SIZE);
    memset(temp_buffer_2, 0, MAX_PATH_SIZE);

    result = GetFullDirPath(temp_buffer_1, MAX_PATH_SIZE, MainOutputDir);
    if (result != 0)
    {
      printf ("Strange Main output dir invalid at this point %s", MainOutputDir);
      strlcpy (temp_buffer_1, MainOutputDir, sizeof(temp_buffer_1));
    }

    result =  GetFullDirPath(temp_buffer_2, MAX_PATH_SIZE, cwd);
    if (result != 0)
    {
      printf("Current working directory invalid, Impossible but true! %s", cwd);
      strlcpy (temp_buffer_2, cwd, sizeof(temp_buffer_2));
    }
    strlcat (temp_buffer_2, "readback"SLASHSTR, sizeof(temp_buffer_2));

    /* compare temp_buffer_1 with temp_buffer_2 to check if absolute_port_trace_name is cwd/readback
       and check if absolute_port_trace_name and final_port_trace_name are not pointing to same location */
    if ((strncmp(absolute_port_trace_name, final_port_trace_name, sizeof(absolute_port_trace_name)) != 0)
        && (strncmp(temp_buffer_1, temp_buffer_2, sizeof(temp_buffer_1)) == 0))
    {
      if(abs_path_exist)
      {
        // absolute_port_name file exist
        fp = fopen (final_port_trace_name, "rb");
        if(fp)
        {
          fclose(fp);
          fp=NULL;
          /* for rename/move to succeed on windows, final_port_trace_name
             should be deleted move fails if destination file already exists */
          if (remove(final_port_trace_name)!=0)
          {
            printf("\n\nDeleting '%s' failed\n\n",final_port_trace_name);
            exit(ExitCode);
          }
        }
        if (rename(absolute_port_trace_name, final_port_trace_name) == 0)
        {
          log_path = final_port_trace_name;
          printf("\nMoved '%s' to '%s'",absolute_port_trace_name, final_port_trace_name);
        }
        else
        {
          log_path = absolute_port_trace_name;
          printf("\nMoving '%s' to '%s' failed",absolute_port_trace_name, final_port_trace_name);
        }
      }
    }
  }
 ...
 *
 */
void string_replace(char *in, size_t len, char *find, char *new_str)
{
  ...
  if (new_len > find_len)
  {
    FH_LOGE("Invalid New string lengths %d > find len %d", new_len, find_len);
    return;
  }
  ...
  while (ahead <= len)
  {
    ...
    {
      memscpy (tail, len - ahead, new_str, new_len);
      ahead += find_len;
      behind += new_len;
    }
    else
...
int json_move_to_char(char *json, char next_c)
{
  int json_len;
  int i;
...
int json_add_xml_to_list(char *start, char *end)
{
  int offset;
  char *curr;
  curr = start;
...
int json_add_file_to_list(char *start, char *end)
{
  int offset;
  char *curr;

  if (global_file_path_list == NULL)
    global_file_path_list = file_path_init();
...
void metacli_json_in(char *json_path)
{
  ...
  if (json_fp == NULL)
  {
    FH_LOGE("Failed to open JSON formatted input file %s", json_path);
    ExitAndShowLog (1);
    return;
  }

  result = fstat (fileno(json_fp), &sbuf);
  if (result != 0)
  {
    FH_LOGE("Failed to get json file size %s", json_path);
    ExitAndShowLog (1);
    return;
  }
  json_len = sbuf.st_size;

  json = (char *)malloc(json_len);
  if (json == NULL)
  {
    FH_LOGE("Failed to allocate JSON memory of length %ld", json_len);
    fclose(json_fp);
    ExitAndShowLog (1);
    return;
  }
  ...
  do{
    read_size = fread(json, 1, json_len - tot_read, json_fp);
    tot_read += read_size;
  } while(read_size > 0);

  fclose(json_fp);
  if ((long int)tot_read < json_len)
  {
    FH_LOGW("File size %d was larger than total data read %llu from json file.",
            json_len, (unsigned long long)tot_read);
    json_len = tot_read;
  }
  ...
  string_replace(json, json_len, "\\\\", "\\");
  {
    ...
    if (offset == -1)
    {
      FH_LOGE("Failed to find open bracket { for json file %d", offset);
      goto error;
    }
    ...
    while (curr <= limit && done == 0)
    {
      ...
      if (offset == -1)
      {
        FH_LOGE("Failed to find opening \" on json input %d", offset);
        goto error;
      }
      ...
      if (end == NULL)
      {
        FH_LOGE("Failed to find closing \" starting at offset %d", offset+1);
        goto error;
      }
      *end = '\0';
      ...
      {
        FH_LOGE("JSON file format not as expected, failed to path array %d", offset);
        goto error;
      }
      *close = '\0';
  ...
  FH_LOGI("Successfully loaded %d file paths from json input", file_path_count(global_file_path_list));
  free(json);
  return;
...
struct sparse_handle_t * sparse_open (FILE * fd, uint64_t current_start_sector, uint64_t sector_size_in_bytes)
{
    ...
    {
        dbg (LOG_ERROR, "Malloc for return sparse handle failed", DUMMY_ARG);
        ExitAndShowLog (1);
    }
    ...
    return_sparse_handle->group_file_offset = malloc (sparse_header.total_chunks*sizeof (long long int) );
    ...
    {
        dbg (LOG_ERROR, "Malloc in sparse_open failed", DUMMY_ARG);
        ExitAndShowLog (1);
    }
    ...
    for (i = 0; (size_t)i < sparse_header.total_chunks; i++)
    {
        ...
        else if (chunk_header.chunk_type == CHUNK_TYPE_FILL)
        {
          ...
          //A new FILL type chunk. Each FILL type chunk considered to be a new unsparsed file.
          start_new_chunk = TRUE;
...
size_t sparse_read (char * data, size_t num_bytes, FILE * fd, struct sparse_read_info_t * sparse_read_struct, struct sparse_header_t sparse_header)
{
  ...
  while (length > 0)
  {
    ...
    {
      uint32_t temp;
      if (fread (&temp, sizeof (char), 4, fd) != 4)
      {
      ...
      for(i = 0; i < chunk/4; i++)
      {
        if(memscpy((data + data_offset + (i * 4)), 4, &temp, 4) != 4)
        {
          dbg (LOG_ERROR, "Error when trying to read %llu bytes of data in sparse_read\n", chunk);
          ExitAndShowLog (1);
        }
...
int filter_label_add_entries(struct multiple_label_filter *data, char *param)
{
  ...
  while (*curr != '\0')
  {
    ...
    if(chunk[1] == '/')
    {
      ...
      {
        dbg (LOG_ERROR, "Invalid LUN number %c for label %s", chunk[0], start);
        return -1;
      }
    }
    else
...
void addSlotAttribute(void)
{
  ...
  {
    if (strstr(XMLStringTable[k], "<configure ") != NULL ||
        strstr(XMLStringTable[k], "<nop ") != NULL ||
        strstr(XMLStringTable[k], "<power ") != NULL ||
        strstr(XMLStringTable[k], "<peek ") != NULL ||
        strstr(XMLStringTable[k], "<poke ") != NULL)
    {
      k++;
      continue;
    }
    ...
    if (pch == NULL)
    {
      ...
      if (pch == NULL)
      {
        dbg(LOG_ERROR, "Missing />", DUMMY_ARG);
        ExitAndShowLog(1);
      }

      snprintf(pch, sizeof(XMLStringTable[k]) - TempLength, "slot=\"%"SIZE_T_FORMAT"\" /></data>", fh.attrs.slot);

    }
    else
    {
      snprintf(pch, sizeof(XMLStringTable[k]) - TempLength, "slot=\"%"SIZE_T_FORMAT"\"", fh.attrs.slot);
    }
...
int main (int argc, char *argv[])
{
    ...
    */
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        else if ((res=strncmp(MyOpt, "mainoutputdir=", MAX(strlen(MyOpt), strlen("mainoutputdir=")))) == 0)
        {
          int result;
          if (res==0)
          {
          ...
          result = GetFullDirPath(FullMainOutputDirPath, MAX_PATH_SIZE, MainOutputDir);
          if (result != 0)
          {
            dbg (LOG_ERROR, "Invalid path '%s'", MainOutputDir);
            ExitAndShowLog (1);
          }
          len = (strlen(FullMainOutputDirPath)>strlen(cwd))?strlen(FullMainOutputDirPath):strlen(cwd);
          CopyString (MainOutputDir, FullMainOutputDirPath, 0, 0,
                      strlen (FullMainOutputDirPath),
                      MAX_PATH_SIZE, strlen (FullMainOutputDirPath));
    ...
    {
      getoptarg (i, argv, MyOpt, sizeof (MyOpt), MyArg, sizeof (MyArg) );
      if (strncmp (MyOpt, "verify_programming", MAX (strlen (MyOpt), strlen ("verify_programming"))) == 0)
      {
        /* If the output dir is CWD, create a readback when using verify programming */
        if ((strlen(MainOutputDir)>0) && strncmp (MainOutputDir, cwd, len) == 0)
        {
          int result;
          snprintf(MainOutputDir+len,10,"readback%s","/");
          memset(FullMainOutputDirPath,'\0',MAX_PATH_SIZE);
          result = GetFullDirPath(FullMainOutputDirPath, MAX_PATH_SIZE, MainOutputDir);
          if (result != 0)
          {
            dbg (LOG_ERROR, "Invalid path '%s'", MainOutputDir);
    ...
    if (MKDIR (MainOutputDir) == -1)
    {
      if (errno == EEXIST)
      {
        dbg(LOG_INFO, "Output Dir Already exists %s", MainOutputDir);
      }
      else
      {
        dbg(LOG_ERROR, "Failed to create ouput dir %s, errno: %d", MainOutputDir, errno);
        ExitAndShowLog(1);
      }
    }
    else
    {
      dbg(LOG_INFO, "Output Dir Created successfully %d", MainOutputDir);
    }
    ...
    dbg (LOG_INFO, "Showing network mappings to allow debugging", DUMMY_ARG);
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        if ( strncmp (MyOpt, "memoryname=", MAX (strlen (MyOpt), strlen ("memoryname=") ) ) == 0 )
        {
            ...
            {
                if ( CopyString (fh.attrs.MemoryName, "NVMe", 0, 0, strlen ("NVMe"), sizeof (fh.attrs.MemoryName), sizeof ("NVMe") )  == 0)
                {
                    dbg (LOG_ERROR, "Failed to copy '%s' of length %"SIZE_T_FORMAT" bytes into fh.attrs.MemoryName", "NVMe", strlen ("NVMe") );
                    ExitAndShowLog (1);
                }
            }
            else if (MyArg[0] == 'n' && MyArg[1] == 'u')
            {
                if ( CopyString (fh.attrs.MemoryName, "null", 0, 0, strlen ("null"), sizeof (fh.attrs.MemoryName), sizeof ("null") )  == 0)
                {
                    dbg (LOG_ERROR, "Failed to copy '%s' of length %"SIZE_T_FORMAT" bytes into fh.attrs.MemoryName", "null", strlen ("null") );
                    ExitAndShowLog (1);
                }
            }
            else
        ...
        if ( strncmp (MyOpt, "labels=", MAX (strlen (MyOpt), strlen ("labels=") ) ) == 0 )
        {
          ...
          if(result != 0)
          {
            dbg (LOG_ERROR, "Invalid string used for labels: %s", cmd_arg);
            ExitAndShowLog (1);
          }
          continue;
        }
        ...
        if ( strncmp (MyOpt, "notlabels=", MAX (strlen (MyOpt), strlen ("notlabels=") ) ) == 0 )
        {
          ...
          if(result != 0)
          {
            dbg (LOG_ERROR, "Invalid string used for notlabels: %s", cmd_arg);
            ExitAndShowLog (1);
          }
          continue;
        }
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        {
            dbg (LOG_WARN, "--createconfigxml parameter is deprecated and scheduled to be removed", DUMMY_ARG);
            createconfigxml = 1;
            Simulate = 1;
        }
        else if ( strncmp (MyOpt, "flattenbuildto=", MAX (strlen (MyOpt), strlen ("flattenbuildto=") ) ) == 0 )
        {
            ...
            if (MKDIR (flattenbuildto) == -1)
                dbg (LOG_INFO, "Already exists", DUMMY_ARG);
            else
                dbg (LOG_INFO, "Created successfully", DUMMY_ARG);
        ...
        {
            dbg (LOG_WARN, "--erasefirst parameter is deprecated and scheduled to be removed", DUMMY_ARG);
            ...
            else
            {
                dbg (LOG_ERROR, "\n\nIt looks like you chose --erasefirst with MemoryName='%s'.\nThis is a --memoryname=nand feature!!"
                     ...
                     "\n\n\nYou don't want this feature and you are looking for e--erase=\n\n"
                     "\n\n\t\tSomething like:    fh_loader.exe --erase=0 --port= etc etc\n\n"
    ...
    if (NumContensXML > 0)
    {
        // to be here means ParseContentsXML(InFileListFilename) returned something, so fill in the rawprograms and search_paths etc
        for (i = 0; i < NumContensXML; i++)
        {
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        else if (strncmp(MyOpt, "port_type=", MAX(strlen(MyOpt), strlen("port_type="))) == 0)
        {
          struct port_types_cmd_line
          {
            char *name;
          ...
          int i;
          int num_port_types;
          struct port_types_cmd_line port_types[] = {{"usb",  FH_TRANSPORT_COM},
                                                     {"pcie", FH_TRANSPORT_COM},
                                                     {"hsuart", FH_TRANSPORT_HSUART},
                                                     {"pipe_test", FH_TRANSPORT_LINUX_PIPE_TEST},
                                                    };
          num_port_types = sizeof(port_types) / sizeof (struct port_types_cmd_line);
          for (i = 0; i < num_port_types; i++)
          {
            char* com_type = port_types[i].name;
            int res = strncasecmp(MyArg, com_type, MAX(strlen(MyArg), strlen(com_type)));
            if (res == 0)
            {
              transport_type = port_types[i].type;
              break;
            }
          }
          ...
          if (i == num_port_types)
          {
            dbg(LOG_INFO, "Invalid port_type specified, possible values are:", DUMMY_ARG);
            for (i = 0; i < num_port_types; i++)
              dbg(LOG_INFO, "%s", port_types[i].name);
            ExitAndShowLog(1);
          }
        }
        else if (strncmp (MyOpt, "loglevel=", MAX (strlen (MyOpt), strlen ("loglevel=") ) ) == 0)
        {
            ...
            {
                dbg (LOG_ALWAYS, "User requested *minimal* logging with --loglevel=0", DUMMY_ARG);
            ...
            {
                dbg (LOG_ALWAYS, "User requested *verbose* logging with --loglevel=2", DUMMY_ARG);
            ...
            {
                dbg (LOG_ALWAYS, "User requested *super verbose* logging with --loglevel=3", DUMMY_ARG);
                VerboseLevel  = LOG_DEBUG;  // Everything is shown
            ...
            {
                dbg (LOG_ALWAYS, "User requested *semi-verbose* logging with --loglevel=1 - This is default", DUMMY_ARG);
        ...
        {
            dbg (LOG_WARN, "--interactive= parameter is deprecated and scheduled to be removed", DUMMY_ARG);
        ...
        else if (strncmp (MyOpt, "trials=", MAX (strlen (MyOpt), strlen ("trials=") ) ) == 0)
        {
            ...
            dbg (LOG_WARN, "--trials= parameter is deprecated and scheduled to be removed", DUMMY_ARG);
        ...
        {
            dbg (LOG_WARN, "--noreset parameter is deprecated and scheduled to be removed", DUMMY_ARG);
            AllowReset = 0; // won't send <power DelayInSeconds="2" value="reset" /> anymore
            continue;
        }
        else if ( strncmp (MyOpt, "reset", MAX (strlen (MyOpt), strlen ("reset") ) ) == 0 )
        {
            dbg (LOG_WARN, "--reset parameter is deprecated and scheduled to be removed", DUMMY_ARG);
        ...
        else if ( strncmp (MyOpt, "power=", MAX (strlen (MyOpt), strlen ("power=") ) ) == 0 )
        {
          ...
          if (split == NULL)
          {
            dbg (LOG_ERROR, "Invalid command line option %s expect --power=<Option>,time_in_seconds, for example --power=reset,1 to reset the device after a 1 second delay.", MyArg);
            ExitAndShowLog (1);
          }
          *split = '\0';
          power_opt = MyArg;
          ...
          {
            dbg (LOG_ERROR, "Invalid power selection for time in seconds %s expected decimal value greater or equal to 0 and less than %d", power_time, FH_MAX_DEVICE_DELAY_FOR_POWER_OP);
            ExitAndShowLog (1);
          }
        ...
        {
            dbg(LOG_INFO, "User wants to change slot", DUMMY_ARG);
            dbg(LOG_INFO, "This will only affect the following TAGS : <program>, <read>, <getstorageinfo>\n", DUMMY_ARG);
        ...
        {
            dbg (LOG_WARN, "--dontsorttags parameter is deprecated and scheduled to be removed", DUMMY_ARG);
        ...
        {
            dbg (LOG_WARN, "--showdigestperpacket parameter is deprecated and scheduled to be removed", DUMMY_ARG);
        ...
        {
            dbg (LOG_WARN, "--showdigestperfile parameter is deprecated and scheduled to be removed", DUMMY_ARG);
        ...
        {
            dbg (LOG_WARN, "--createcommandtrace parameter is deprecated and scheduled to be removed", DUMMY_ARG);
        ...
        {
            dbg (LOG_DEBUG, "User wants to Verify flashing/programming using the getsha256digest command\n", DUMMY_ARG);
            verify_programming = 1;
        ...
        else if (strncmp (MyOpt, "createdigests", MAX (strlen (MyOpt), strlen ("createdigests") ) ) == 0)
        {
            ...
            dbg (LOG_WARN, "--createdigests parameter is deprecated and scheduled to be removed, use --createvipdigests= instead.", DUMMY_ARG);
            CreateDigests   = 1;
            Simulate      = 1;
        ...
        else if (strncmp (MyOpt, "createvipdigests=", MAX (strlen (MyOpt), strlen ("createvipdigests") ) ) == 0)
        {
          ...
          {
            char* com_type = header_types[i].name;
            int res = strncasecmp(MyArg, com_type, MAX(strlen(MyArg), strlen(com_type)));
            if (res == 0)
            {
              digest_header_type = header_types[i].type;
              break;
            }
          }
          ...
          if (i == num_types)
          {
            dbg(LOG_INFO, "Invalid VIP digest specified in createvipdigests, possible values are:", DUMMY_ARG);
            for (i = 0; i < num_types; i++)
              dbg(LOG_INFO, "%s", header_types[i].name);
            ExitAndShowLog(1);
          }
        ...
        else if (strncmp (MyOpt, "skippatch", MAX (strlen (MyOpt), strlen ("skippatch") ) ) == 0)
        {
            ...
            dbg (LOG_WARN, "--skippatch parameter is deprecated and scheduled to be removed", DUMMY_ARG);
        ...
        else if (strncmp (MyOpt, "skipstorageinit", MAX (strlen (MyOpt), strlen ("skipstorageinit") ) ) == 0)
        {
            ...
            dbg (LOG_WARN, "--skipstorageinit parameter is no longer needed for targets after MSM8998", DUMMY_ARG);
        ...
        else if (strncmp (MyOpt, "readbogusdata", MAX (strlen (MyOpt), strlen ("readbogusdata") ) ) == 0)
        {
            ...
            dbg (LOG_WARN, "--readbogusdata parameter is deprecated and scheduled to be removed", DUMMY_ARG);
        ...
        else if (strncmp (MyOpt, "comportopentimeout=", MAX (strlen (MyOpt), strlen ("comportopentimeout=") ) ) == 0)
        {
            ...
            dbg (LOG_WARN, "--comportopentimeout= parameter is deprecated and scheduled to be removed", DUMMY_ARG);
            continue;
        }
        else if (strncmp (MyOpt, "maxdigesttablesizeinbytes=", MAX (strlen (MyOpt), strlen ("maxdigesttablesizeinbytes=") ) ) == 0)
        {
            dbg (LOG_WARN, "--maxdigesttablesizeinbytes= parameter is deprecated and scheduled to be removed", DUMMY_ARG);
        ...
        {
          dbg (LOG_WARN, "--forceoverwrite parameter is deprecated and scheduled to be removed", DUMMY_ARG);
          ForceOverwrite = 1;
        }
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        {
            dbg (LOG_WARN, "--getgptmainbackup= parameter is deprecated and scheduled to be removed", DUMMY_ARG);
            InitBufferWithXMLHeader (tx_buffer, sizeof (tx_buffer) );
            AppendToBuffer (tx_buffer, "<data>\n", FIREHOSE_TX_BUFFER_SIZE);
        ...
        {
            dbg (LOG_INFO, "User requested forcecontentsxmlpaths", DUMMY_ARG);
            forcecontentsxmlpaths = 1;
        }
        else if (strncmp (MyOpt, "verify_build", MAX (strlen (MyOpt), strlen ("verify_build") ) ) == 0)
        {
            dbg (LOG_WARN, "--verify_build parameter is deprecated and scheduled to be removed", DUMMY_ARG);
            dbg (LOG_INFO, "User requested verify_build, NOTE: This is to be used with --contentsxml=", DUMMY_ARG);
            verify_build = 1;
            Simulate   = 1;
            AllowReset   = 0; // won't send <power DelayInSeconds="2" value="reset" /> anymore
        ...
        {
          dbg (LOG_ERROR, "benchmarkddr option is no longer supported.", DUMMY_ARG);
            continue;
        }
        ...
        else if (strncmp (MyOpt, "stresstest", MAX (strlen (MyOpt), strlen ("stresstest") ) ) == 0)
        {
            ...
            dbg (LOG_WARN, "--stresstest= parameter is deprecated and scheduled to be removed", DUMMY_ARG);
    ...
    if (Simulate)
    {
        dbg (LOG_ALWAYS, "Remember: This *is* a simulation. Nothing will be sent to target, COM PORT NOT OPENED", DUMMY_ARG);
        dbg (LOG_ALWAYS, "      _                 _       _       ", DUMMY_ARG);
        dbg (LOG_ALWAYS, "     (_)               | |     | |      ", DUMMY_ARG);
        dbg (LOG_ALWAYS, "  ___ _ _ __ ___  _   _| | __ _| |_ ___ ", DUMMY_ARG);
        dbg (LOG_ALWAYS, " / __| | '_ ` _ \\| | | | |/ _` | __/ _ \\", DUMMY_ARG);
        dbg (LOG_ALWAYS, " \\__ \\ | | | | | | |_| | | (_| | ||  __/", DUMMY_ARG);
    ...
    else
    {
        ...
        {
            dbg (LOG_ERROR, "Port device name not specified; Example: --port=\\\\.\\COM19 ", DUMMY_ARG);
            ExitAndShowLog (1);
        }
        ...
        if (ReadBogusDataBeforeSendingConfigure)
        {
            ...
            dbg (LOG_ALWAYS, "**Attempting to read 4KB to flush out any possible strange first log packet since user chose ReadBogusDataBeforeSendingConfigure", DUMMY_ARG);
...
//  PRETTYPRINT(buffer, BytesWritten);
    ...
    if (SortTags)
    {
        ...
        dbg (LOG_INFO, "Sorting TAGS to ensure order is <configure>,<erase>, others, <patch>,<power>", DUMMY_ARG);
        SortMyXmlFiles();
    }
    else
        dbg (LOG_INFO, "*Not* sorting tags since --dontsorttags was provided\n", DUMMY_ARG);

    if (strlen (SignedDigestTableFilename) != 0 && strlen (ChainedDigestTableFilename) != 0)
    {
      fh_res_t res;
      if (fh_transfer_get_type() != FH_TRANSPORT_NONE)
      {
        res = fh_transfer_vip (SignedDigestTableFilename, ChainedDigestTableFilename, MaxNumDigestsPerTable);
        if (res != SUCCESS)
          ExitAndShowLog (1);
      }
    }
    ...
    {
        fd = ReturnFileHandle (DigestsPerFileName, strlen (DigestsPerFileName), "wb"); // will exit if not successful
        fclose (fd);
        fd = NULL;
    }
    ...
    dbg (LOG_ALWAYS, "==============================================================", DUMMY_ARG);
    dbg (LOG_ALWAYS, "Files used and their paths", DUMMY_ARG);
    ...
    if (strlen (WarningsBuffer) > 0)
    {
        ...
        dbg (LOG_ALWAYS, "==============================================================", DUMMY_ARG);
        dbg (LOG_ALWAYS, "NOTE: There were WARNINGS!! Repeated here, but please see log for more detail", DUMMY_ARG);
    ...
    if (Simulate)
    {
        dbg (LOG_ALWAYS, "Remember: This was a simulation. Nothing was sent to a target", DUMMY_ARG);
        dbg (LOG_ALWAYS, "      _                 _       _       ", DUMMY_ARG);
        dbg (LOG_ALWAYS, "     (_)               | |     | |      ", DUMMY_ARG);
        dbg (LOG_ALWAYS, "  ___ _ _ __ ___  _   _| | __ _| |_ ___ ", DUMMY_ARG);
        dbg (LOG_ALWAYS, " / __| | '_ ` _ \\| | | | |/ _` | __/ _ \\", DUMMY_ARG);
    ...
    dbg (LOG_ALWAYS, "     _             (done)", DUMMY_ARG);
    dbg (LOG_ALWAYS, "    | |                 ", DUMMY_ARG);
    dbg (LOG_ALWAYS, "  __| | ___  _ __   ___ ", DUMMY_ARG);
    dbg (LOG_ALWAYS, " / _` |/ _ \\| '_ \\ / _ \\", DUMMY_ARG);
    dbg (LOG_ALWAYS, "| (_| | (_) | | | |  __/", DUMMY_ARG);
    dbg (LOG_ALWAYS, " \\__,_|\\___/|_| |_|\\___|", DUMMY_ARG);
...
uint8 elf_header_first_96bytes[96] = {
...
void PrepareDigestTable (void)
{
    ...
    if (DigestFile == NULL)
    {
        dbg (LOG_ERROR, "'%s' not found", "DIGEST_TABLE.bin", DUMMY_ARG);
        ExitAndShowLog (1);
    }
    ...
    if (Remainder > 0)
    {
        dbg (LOG_ERROR, "Your DIGEST TABLE '%s' file is not a multiple of 32 bytes",  "DIGEST_TABLE.bin", DUMMY_ARG);
        ExitAndShowLog (1);
    }
    ...
    {
        dbg (LOG_INFO, "3. In PrepareDigestTable AND MaxNumDigestsPerTable WAS ZERO", DUMMY_ARG);
        MaxNumDigestsPerTable = fh.attrs.MaxDigestTableSizeInBytes / DigestSizeInBytes;
    }
    ...
    if (NumDigestsInFile >= 54)
      ...
      first_table_size = (NumDigestsInFile + 1) * 32; /* add 1 hash for all zero hash at the end */
    ...
    if (digest_header_type == FH_DIGEST_HEADER_MBN)
    {
      ...
      fwrite (&first_table_size, 4, 1, FTemp); // Size of complete image in bytes
      fwrite (&first_table_size, 4, 1, FTemp); // Size of code region of image in bytes
    ...
    {
      uint8 region_size[8];
      uint8 elf_header_last_bytes[ELF_HEADER_SIZE - sizeof(elf_header_first_96bytes) - 2 * 8];
      FH_LOGI("HEADER ELF %d", 0);
      /* Newer targets require an ELF header. Use the same inital table, add an ELF header instead. */
      FTemp = ReturnFileHandle ("DigestsToSign.bin.elf", strlen ("DigestsToSign.bin.elf"), "wb"); // will exit if not successful
      fwrite (elf_header_first_96bytes, sizeof(elf_header_first_96bytes), 1, FTemp); /* Initial ELF header */
      ...
      fwrite (region_size, sizeof(region_size), 1, FTemp);
      fwrite (region_size, sizeof(region_size), 1, FTemp);

      /* ELF Header is 232 bytes in size, write the end which is all zeros for this blob. */
      memset (elf_header_last_bytes, 0, sizeof(elf_header_last_bytes));
      fwrite (elf_header_last_bytes, sizeof(elf_header_last_bytes), 1, FTemp);
    }
    ...
    {
      FH_LOGE("Invalid selection of VIP digest header: %d", (int)digest_header_type);
      ExitAndShowLog (1);
    }
    ...
    {
        dbg (LOG_ERROR, "This should not be hit. The Digest output file is a multiple of 512. This will cause the target to hang. Please re-run and add --nop to fix this issue", DUMMY_ARG);
        ExitAndShowLog (1);
    }
...
void TestIfWeNeedToSendDigestTable (void)
{
    ...
    /* Check if the transfer layer is enabled, if it was it will take care of
       sending VIP data. */
    if (fh_transfer_get_type() != FH_TRANSPORT_NONE)
      return;
    ...
    dbg (LOG_DEBUG, "\t _               _       _        _     _      ", DUMMY_ARG);
    dbg (LOG_DEBUG, "\t| |             | |     | |      | |   | |     ", DUMMY_ARG);
    dbg (LOG_DEBUG, "\t| |__   __ _ ___| |__   | |_ __ _| |__ | | ___ ", DUMMY_ARG);
    dbg (LOG_DEBUG, "\t| '_ \\ / _` / __| '_ \\  | __/ _` | '_ \\| |/ _ \\", DUMMY_ARG);
    dbg (LOG_DEBUG, "\t| | | | (_| \\__ \\ | | | | || (_| | |_) | |  __/", DUMMY_ARG);
    dbg (LOG_DEBUG, "\t|_| |_|\\__,_|___/_| |_|  \\__\\__,_|_.__/|_|\\___|", DUMMY_ARG);
    ...
    if (FileSize == 0)
    {
        dbg (LOG_INFO, "--chaineddigests file was 0 bytes, not sending any chained digest tables", DUMMY_ARG);
        return;
    }
    ...
    if (GotACK)
    {
        dbg (LOG_DEBUG, "Got the ACK", DUMMY_ARG);
        dbg (LOG_DEBUG, "DIGEST TABLE ACCEPTED - Just RESET CurrentDigestLocation to 0, since now using a new table", DUMMY_ARG);
        CurrentDigestLocation = 0;
        digest_file_offset += MaxNumDigestsPerTable * DigestSizeInBytes;
    }
    else
    {
        dbg (LOG_ERROR, "Digest Table was NOT ACCEPTED. Can't continue", DUMMY_ARG);
        ExitAndShowLog (1);
    }
}
...
void SendSignedDigestTable (char *SignedDigestTableFilename)
{
    ...
    dbg (LOG_INFO, "\t _____ _                   _                  ", DUMMY_ARG);
    dbg (LOG_INFO, "\t/  ___(_)                 | |                 ", DUMMY_ARG);
    dbg (LOG_INFO, "\t\\ `--. _  __ _ _ __   __ _| |_ _   _ _ __ ___ ", DUMMY_ARG);
    dbg (LOG_INFO, "\t `--. \\ |/ _` | '_ \\ / _` | __| | | | '__/ _ \\", DUMMY_ARG);
    dbg (LOG_INFO, "\t/\\__/ / | (_| | | | | (_| | |_| |_| | | |  __/", DUMMY_ARG);
    dbg (LOG_INFO, "\t\\____/|_|\\__, |_| |_|\\__,_|\\__|\\__,_|_|  \\___|", DUMMY_ARG);
    dbg (LOG_INFO, "\t          __/ |                               ", DUMMY_ARG);
    dbg (LOG_INFO, "\t         |___/   \n", DUMMY_ARG);
    dbg (LOG_INFO, "\t______                      _ ", DUMMY_ARG);
    dbg (LOG_INFO, "\t| ___ \\                    | |", DUMMY_ARG);
    dbg (LOG_INFO, "\t| |_/ /_ _ ___ ___  ___  __| |", DUMMY_ARG);
    dbg (LOG_INFO, "\t|  __/ _` / __/ __|/ _ \\/ _` |", DUMMY_ARG);
    dbg (LOG_INFO, "\t| | | (_| \\__ \\__ \\  __/ (_| |", DUMMY_ARG);
    dbg (LOG_INFO, "\t\\_|  \\__,_|___/___/\\___|\\__,_|\n", DUMMY_ARG);
    dbg (LOG_DEBUG, "FIREHOSE: Signed DIGEST sent. This is not counted towards command sent", DUMMY_ARG);
    dbg (LOG_DEBUG, "FIREHOSE: PacketsSent reset to 0", DUMMY_ARG);
...
void SetPortTimeouts (void)
{
    ...
    {
        dbg (LOG_ERROR, "Error setting com port timeouts", DUMMY_ARG);
        ExitAndShowLog (1);
    }
...
unsigned int WritePort (unsigned char *pData, unsigned int length, unsigned int MaxLength, unsigned char RawData)
{
    ...
    enum fh_log_lvl LogType = LOG_DEBUG;
    int ret = 0;
    char answer = 'y';
    ...
    else
    {
        ...
        for (i = 0; i < NumTries; i++)
        {
            ...
            {
              int bytes_written;
              bytes_written = write (port_fd, pData, length);
    ...
    if (BytesWritten == 0)
    {
        dbg (LOG_ERROR, "WriteFile failed, reported 0 bytes written", DUMMY_ARG);
        ExitAndShowLog (1);
        return 0;
    }
    ...
    /*
      if(BytesWritten!=length)
      {
        dbg(LOG_ERROR, "(P%"SIZE_T_FORMAT4") (H%"SIZE_T_FORMAT5", DUMMY_ARG) Tried to write %7d bytes, but only wrote %d bytes) ---------->\n\n\n",PacketsSent,CurrentDigestLocation,length,BytesWritten);
        ExitAndShowLog(1);
        return 0;
      }
    */
...
unsigned int ReadPort (unsigned char *pData, unsigned int length, unsigned int MaxLength)
{
    ...
    rx_res = fh_rx_blocking_raw(pData, length, &bytes_read);
    if (rx_res != NOT_INITALIZED)
    {
      return bytes_read;
    }

    /* Transport layer was not initialized, fall back to older code. */
    if (length > MaxLength)
    {
        length = MaxLength;
    ...
    for (i = 0; i < NumTries; i++)
    {
        ...
        if (retval == 0)
        {
            dbg (LOG_INFO, "Timeout - no response trying to read from target\n\n", DUMMY_ARG);
            return 0;
        }
...
unsigned int OpenPort (char *pData)
{
    ...
    double Elapsed;
    fh_res_t transfer_res;
    static int fh_transport_has_inited = 0;

    if (transport_type != FH_TRANSPORT_NONE && fh_transport_has_inited == 0)
    {
      fh_transfer_init(transport_type);
      fh_transport_has_inited = 1;
    }
    transfer_res = fh_transfer_open(pData);
    if (transfer_res == SUCCESS)
    {
      return 0;
    }
    else if (transfer_res != NOT_INITALIZED)
    {
      return 1;/* Transport layer was initialized and reported error */
    }
...
{
  fh_res_t transfer_res;
  transfer_res = fh_transfer_close();
  if (transfer_res == SUCCESS || transfer_res != NOT_INITALIZED)
  {
    return;
  }
...
{
  printBuffer((const unsigned char *)buffer, (unsigned int)len, (unsigned int)len, "", 0);
}
...
void printBuffer (const unsigned char *buffer, unsigned int length, unsigned int MaxLength, const char *func_name, int line_number)
{
    ...
    //dbg(LOG_FILE_ONLY, "PRETTYPRINT %s:%d Buffer is %d bytes\n", func_name, line_number, length);
    dbg (LOG_FILE_ONLY, "PRETTYPRINT Buffer is %d bytes\n", length);
    ...
    while (1)
    {
        ...
        dbg (LOG_FILE_ONLY, szBuffer, DUMMY_ARG);
...
void AppendToBuffer (char *MyBuffer, char *buf, SIZE_T MaxBufferSize)
{
    ...
    memscpy ( (MyBuffer + Length), (size_t)MaxBufferSize, buf, (size_t)CharsToAdd); // memcpy
...
{
  fh_res_t tx_res;
  tx_res = fh_tx_blocking(tx_buffer, Length);
  if (tx_res == SUCCESS)
  {
    return Length;
  }
  else if (tx_res != NOT_INITALIZED)
  {
    return 0;
  }
...
unsigned int sendTransmitBuffer (void)
{
  ...
  tx_res = fh_tx_blocking(tx_buffer, BytesBeingSent);
  if (tx_res == SUCCESS)
  {
    dbg (LOG_DEBUG, "CHANNEL DATA (P%"SIZE_T_FORMAT4") (H%"SIZE_T_FORMAT5", DUMMY_ARG) (%7lld bytes) - HOST TO TARGET -->\n"
         "%s",
         PacketsSent++, CurrentDigestLocation++, BytesBeingSent, tx_buffer);

    return BytesBeingSent;
  }
  else if (tx_res != NOT_INITALIZED)
  {
    return 0;
...
char * RemoveEverythingButTags (char *Packet, SIZE_T  CurrentPacketLoc, SIZE_T MaxPacketSize)
{
    ...
    {
        dbg (LOG_ERROR, "Couldn't backup Packet into temp_buffer2 before parsing!", DUMMY_ARG);
        ExitAndShowLog (1);
    }
#endif
    ...
    do
    {
        ...
        {
            dbg (LOG_ERROR, "Didn't fine either Left Brace or Right Brace. XML file is not formed correctly", DUMMY_ARG);
            ExitAndShowLog (1);
        }
...
SIZE_T  DetermineTag (char *Packet, SIZE_T  CurrentPacketLoc, SIZE_T MaxPacketSize)
{
    ...
    if (CurrentPacketLoc >= MaxPacketSize)
    {
        dbg (LOG_ERROR, "CurrentPacketLoc>=MaxPacketSize", DUMMY_ARG);
    ...
    if (PacketLength == 0)
    {
        ...
        if (PacketLength == 0)
        {
            dbg (LOG_ERROR, "No XML detected?? Something has gone wrong. Either an XML file you provided has syntax errors, or your target responded with garbage. Cannot continue. Log will give more information", DUMMY_ARG);
            ExitAndShowLog (1);
        }
    }
    ...
    while ( IsEmptySpace ( Packet[CurrentPacketLoc] ) )
    {
        ...
        if (CurrentPacketLoc >= PacketLength)
        {
            dbg (LOG_ERROR, "XML packet not formed correctly. Ran out of room looking for TAG", DUMMY_ARG);
            return 0;
        }
    }
...
SIZE_T  DetermineAttributes (char *Packet, SIZE_T  CurrentPacketLoc, SIZE_T MaxPacketSize)
{
    ...
    while (CurrentPacketLoc < PacketLength)
    {
        ...
        {
            dbg (LOG_ERROR, "XML not formed correctly. Could not find '=' character", DUMMY_ARG);
            return 0; // not formed correctly
        }
        ...
        if ( AssignAttribute (NewAttrName, NewAttrValue, sizeof (NewAttrValue) ) != 0 )
        {
            ...
            {
                dbg (LOG_INFO, "Found NUM_DISK_SECTORS on num_partition_sectors, only allowed for <erase", DUMMY_ARG);
            }
            else
            {
        ...
        while ( IsEmptySpace ( Packet[CurrentPacketLoc] ) )
        {
            ...
            if (CurrentPacketLoc >= PacketLength)
            {
                dbg (LOG_ERROR, "XML packet not formed correctly. Ran out of room looking for ATTRIBUTES", DUMMY_ARG);
...
static firehose_error_t handlePower()
{
    ...
    dbg (LOG_INFO, "Sending <power>", DUMMY_ARG);
...
static firehose_error_t handleSetBootableStorageDrive()
{
    dbg (LOG_INFO, "Sending <setbootablestoragedrive>", DUMMY_ARG);
    // tx_buffer already holds the XML file
    sendTransmitBuffer();
...
static firehose_error_t handleConfigure()
{
    ...
    do
    {
        ...
        dbg (LOG_INFO, "Sending <configure>", DUMMY_ARG);
        ...
        if (!GotACK)
        {
            ...
            if (AllAttributes[1].Assigned == 0)
            {
                ...
                dbg (LOG_INFO, "Something failed. The target rejected your <configure>. Please inspect log for more information", DUMMY_ARG);
            ...
            else
            {
                if (BytesRead == 0)
                {
                    dbg (LOG_ERROR, "Your target is NOT responding!!", DUMMY_ARG);
                    ExitAndShowLog (1);
                }

                dbg (LOG_INFO, "Target returned NAK for your <configure> but it does not seem to be an error. This is ok, fh_loader.exe attributes updated", DUMMY_ARG);
            }
        }


        if (GotACK)
            dbg (LOG_DEBUG, "Got the ACK for the <configure>", DUMMY_ARG);
...
static firehose_error_t handlePatchProgram()  // used for <patch>
{
    ...
    else
    {
        ...
        if (!GotACK)
        {
            dbg (LOG_ERROR, "Something failed during patching. Please see port trace", DUMMY_ARG);
            ExitAndShowLog (1);
            //return FIREHOSE_ERROR;
        }
...
static firehose_error_t handleOptions()
{
    ...
    while (PacketLoc < PacketLength)
    {
        ...
        if (OnKey)
        {
            ...
            {
                dbg (LOG_ERROR, "The key value pair in your <options> section is too big", DUMMY_ARG);
                ExitAndShowLog (1);
            }
        ...
        else
        {
            if (i >= MAX_STRING_SIZE)
            {
                dbg (LOG_ERROR, "The key value pair in your <options> section is too big for Value", DUMMY_ARG);
                ExitAndShowLog (1);
            }
        ...
        if (PairFound)
        {
            ...
            /*
                  if( strncmp(Key,"DOWNLOAD_PROTOCOL",MAX(strlen(Key),strlen("DOWNLOAD_PROTOCOL")))==0 )
                  {
                    ...
                    {
                       dbg(LOG_ERROR, "You have specified a protocol *other* than FIREHOSE. This tool only speaks FIREHOSE", DUMMY_ARG);
                       ExitAndShowLog(1);
                    }
            ...
            */
            if ( strncmp (Key, "LOAD_RAW_PROGRAM_FILES", MAX (strlen (Key), strlen ("LOAD_RAW_PROGRAM_FILES") ) ) == 0 )
            {
                ...
                {
                    dbg (LOG_INFO, "You have specified to NOT load rawprogram files", DUMMY_ARG);
            ...
            if ( strncmp (Key, "LOAD_PATCH_PROGRAM_FILES", MAX (strlen (Key), strlen ("LOAD_PATCH_PROGRAM_FILES") ) ) == 0 )
            {
                ...
                if ( strncmp (Value, "false", strlen (Value) ) == 0 )
                {
                    dbg (LOG_INFO, "You have specified to NOT load patch files", DUMMY_ARG);
                    LOAD_PATCH_PROGRAM_FILES = 0;
                }
    ...
    dbg (LOG_INFO, "Parsed XML Options successfully", DUMMY_ARG);
    return FIREHOSE_SUCCESS;
}
...
static firehose_error_t handleNop()
{
    ...
    dbg (LOG_INFO, "Got the PING response", DUMMY_ARG);
...
static firehose_error_t handleErase()
{
    dbg (LOG_INFO, "                        Issuing Erase                       ", DUMMY_ARG);
    dbg (LOG_INFO, " _____              _               _____                   ", DUMMY_ARG);
    dbg (LOG_INFO, "|_   _|            (_)             |  ___|                  ", DUMMY_ARG);
    dbg (LOG_INFO, "  | | ___ ___ _   _ _ _ __   __ _  | |__ _ __ __ _ ___  ___ ", DUMMY_ARG);
    dbg (LOG_INFO, "  | |/ __/ __| | | | | '_ \\ / _` | |  __| '__/ _` / __|/ _ \\", DUMMY_ARG);
    dbg (LOG_INFO, " _| |\\__ \\__ \\ |_| | | | | | (_| | | |__| | | (_| \\__ \\  __/", DUMMY_ARG);
    dbg (LOG_INFO, " \\___/___/___/\\__,_|_|_| |_|\\__, | \\____/_|  \\__,_|___/\\___|", DUMMY_ARG);
    dbg (LOG_INFO, "                             __/ |                          ", DUMMY_ARG);
    dbg (LOG_INFO, "                            |___/ \n\n", DUMMY_ARG);
    ...
    dbg (LOG_INFO, "\n\n", DUMMY_ARG);
    dbg (LOG_INFO, "ERASE SUCCESSFUL", DUMMY_ARG);
    dbg (LOG_INFO, "ERASE SUCCESSFUL", DUMMY_ARG);
...
firehose_error_t GetNextPacket (void)
{
    ...
    while (TimeToBreak == 0)
    {
        ...
        if (NeedToReadFromChannel)
        {
            ...
            else
            {
                    ...
                    //if(NumTries>=9999999999)  // use this to debug issues with JTAG
                {
                    ...
                    dbg (LOG_ERROR, "Can't write to your target?", DUMMY_ARG);
                    ExitAndShowLog (1);
                }

                //sleep(10);    // use this to debug issues with JTAG
                sleep (1);
                dbg (LOG_INFO, "Back from sleep", DUMMY_ARG);
        ...
        if (PacketLoc == 0)
        {
            dbg (LOG_ERROR, "3. TAG not found or recognized", DUMMY_ARG);
        ...
        if (ParseAttributes)
        {
            ...
            if (PacketLoc == 0)
            {
                dbg (LOG_ERROR, "ATTRIBUTES not found or recognized", DUMMY_ARG);
                return FIREHOSE_ERROR;
            }
        }
        else
...
void LoadPowerOpIntoStringTable (enum fh_reboot_options option, unsigned int seconds)
{
  ...
  snprintf (temp_buffer, sizeof(temp_buffer), "DelayInSeconds=\"%d\" ", seconds);
  AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
  snprintf (temp_buffer, sizeof(temp_buffer), "value=\"%s\" />\n</data>", reboot_str);
  AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
...
void SendReset (void)
{
    dbg (LOG_INFO, "Sending RESET to target", DUMMY_ARG);
...
SIZE_T SplitStringOnCommas (char *sz, SIZE_T SizeOfString, char szArray[][MAX_PATH_SIZE], int offset, SIZE_T ArraySize, SIZE_T StringSize)
{
    ...
    if (k >= ArraySize)
    {
        dbg (LOG_ERROR, "In SplitStringOnCommas the array cannot hold anymore strings", DUMMY_ARG);
        ExitAndShowLog (1);
    }
...
//  }
...

    {
      ...
      if (full_path != NULL)
      {
        dbg (LOG_DEBUG, "2. Calling stat(%s)", full_filename_with_path);
        if (_stat64 (full_path, &status_buf) == 0)
        {
          ...
          //fj = ReturnFileHandle(filename, MAX_PATH_SIZE, "rb"); // will exit if not successful
          fj = ReturnFileHandle (full_path, MAX_PATH_SIZE, "rb"); // will exit if not successful
          LastFindFileFileSize = ReturnFileSize (fj);
          fclose (fj);
          fj = NULL;
...
void StoreXMLFile (char *FileWithPath)
{
    ...
    */
    ...
    if (pch != NULL)
    {
        dbg (LOG_WARN, "Found 'magic_numbers' tag, appears to be a partition_nand.xml, so ignoring this", DUMMY_ARG);
        goto cleanup;
    }
    ...
    while (1)
    {
        ...
        if (PacketLoc == 0)
        {
            dbg (LOG_ERROR, "1. TAG not found or recognized", DUMMY_ARG);
            goto cleanup;
        }
        ...
        strlcpy (single_xml_buffer, xml_buffer + PacketLoc, pch - &xml_buffer[PacketLoc] + 3);
        ...
        if (sparse_pch != NULL && (strcmp (NewTagName, "program") == 0) && fh.attrs.filename[0] != '\0')
        {
            ...
            for (i = 0; (SIZE_T)i < strlen (fh.attrs.filename); i++)
            {
                if (IsEmptySpace (fh.attrs.filename[i]) )
                    fh.attrs.filename[i] = '\0';
            }
...
int GetFullDirPath(char *ResolvedPath, int MaxLength, char *OutputDirPath)
{
  char* outstr = NULL;
  int len = 0;
  len = strlen(OutputDirPath);
  if (len == 0 || len > MaxLength)
  {
    dbg (LOG_ERROR, "Invalid input to GetFullDirPath\n", DUMMY_ARG);
    return -1;
  }

#ifdef _MSC_VER // i.e. if compiling under windows
  outstr = _fullpath(ResolvedPath, OutputDirPath, MaxLength);
  if (outstr == NULL)
  {
    dbg (LOG_ERROR, "Invalid path: '%s'\n",OutputDirPath);
    return -1;
  }
#else
  outstr = realpath(OutputDirPath, ResolvedPath);
  if (outstr == NULL && errno != ENOENT)
  {
    dbg (LOG_ERROR, "Invalid path: '%s'\n",OutputDirPath);
    return -1;
  }
...
/* param  in_or_out_dir  Use input or output dir. 0 for ouput, 1 of input.   */
FILE * ReturnFileHandleSelect (char *filename, long MaxLength, char *AccessMode, enum in_or_out_dir in_or_out_dir)
{
    ...
    if ((strlen (MainOutputDir) > 0) && (in_or_out_dir == OUTPUT_DIR) && ((dir_pch == NULL) && (dir2_pch == NULL))) // user specified this at command line
    {
...
FILE *ReturnFileHandleOutDir (char *filename, long MaxLength, char *AccessMode)
{
  return ReturnFileHandleSelect(filename, MaxLength, AccessMode, OUTPUT_DIR);
}

FILE * ReturnFileHandle (char *filename, long MaxLength, char *AccessMode)
{
  char *w_pch = NULL;
  enum in_or_out_dir dir_search_group = INPUT_DIR;
  w_pch = strchr(AccessMode, 'w'); //check if "w" is in AccessMode
  if (w_pch != NULL)
    dir_search_group = OUTPUT_DIR;
  return ReturnFileHandleSelect(filename, MaxLength, AccessMode, dir_search_group);
}
...
static firehose_error_t handleFirmwareWrite()
{
    ...
    ConvertProgram2Firmware = 1;
    dbg (LOG_INFO, "In handleFirmwareWrite, passing control to handleProgram()", DUMMY_ARG);
    ReturnValue = handleProgram();

    if (ReturnValue == FIREHOSE_SUCCESS)
    {
        dbg (LOG_INFO, "\n", DUMMY_ARG);
        dbg (LOG_INFO, "                      (firmware updated)", DUMMY_ARG);
        dbg (LOG_INFO, "______ ______________  ____    _  ___  ______ _____   _   _____________  ___ _____ ___________ ", DUMMY_ARG);
        dbg (LOG_INFO, "|  ___|_   _| ___ \\  \\/  | |  | |/ _ \\ | ___ \\  ___| | | | | ___ \\  _  \\/ _ \\_   _|  ___|  _  \\", DUMMY_ARG);
        dbg (LOG_INFO, "| |_    | | | |_/ / .  . | |  | / /_\\ \\| |_/ / |__   | | | | |_/ / | | / /_\\ \\| | | |__ | | | |", DUMMY_ARG);
        dbg (LOG_INFO, "|  _|   | | |    /| |\\/| | |/\\| |  _  ||    /|  __|  | | | |  __/| | | |  _  || | |  __|| | | |", DUMMY_ARG);
        dbg (LOG_INFO, "| |    _| |_| |\\ \\| |  | \\  /\\  / | | || |\\ \\| |___  | |_| | |   | |/ /| | | || | | |___| |/ / ", DUMMY_ARG);
...
static firehose_error_t handleProgram()
{
    ...
    int local_temp_buffer_size = 0;
    int pre_verify_programming = 0;
    ...
    if (FileSizeNumSectors == 0)
    {
        dbg (LOG_ERROR, "FileSizeNumSectors is 0. Nothing to program!!", DUMMY_ARG);
        ExitAndShowLog (1);
    }
    ...
    time_throughput_calculate (&AbsoluteTimeStart, &time_end, FileSizeNumSectors * fh.attrs.SECTOR_SIZE_IN_BYTES, -1.0, LOG_ALWAYS );
    ...
    if (pre_verify_programming || ShowDigestPerFile)
    {
        ...
        PrettyPrintHexValueIntoTempBuffer (verify_hash_value, 32, 0, 32); // from, size, offset, maxlength
        printf ("\n");
        ...
        if(GenerateSha256File)
        {
            fd = ReturnFileHandle (DigestsPerFileName, strlen (DigestsPerFileName), "ab" ); // will exit if not successful
            if (fd != NULL)
            {
                fwrite (verify_hash_value, 32, 1, fd);
                fclose (fd);
                fd = NULL;
                return FIREHOSE_SUCCESS;
            }
    ...
    if (!GotACK)
    {
        dbg (LOG_ERROR, "Please see log", DUMMY_ARG);
        ExitAndShowLog (1);
    }
    ...
    else if (verify_programming)
    {
      ...
      if (verify_programming_sha256 == 0)
      {
        ...
        snprintf(temp_buffer, sizeof(temp_buffer), "slot=\"%"SIZE_T_FORMAT"\" ", fh.attrs.slot);
        AppendToBuffer(tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
        ...
        if (Simulate == 0)
        {
          char *readback_file_path = find_file (fh.attrs.filename, 1);
          /* fh.attrs.filename assumes images are in cwd so pick the correct image path to calculate sha */
          dbg (LOG_INFO, "********** for '%s'********", FileToSendWithPath);
          fd = ReturnFileHandleOutDir (fh.attrs.filename, MAX_PATH_SIZE, "rb"); // will exit if not successful
          FileSize = ReturnFileSize (fd);
          ...
          dbg (LOG_INFO, "Calculating SHA256", DUMMY_ARG);
          ...
          {
            dbg (LOG_ERROR, "File size is not a multiple of sector size", DUMMY_ARG);
            ExitAndShowLog(1);
          }
      ...
      if(VerifySha256File)
      {
        ...
        PrettyPrintHexValueIntoTempBuffer (verify_hash_value, 32, 0, 32); // from, size, offset, maxlength
        printf ("\n");
        dbg (LOG_INFO, "verify_programming read SHA256 from '%s'\n'%s'SHA256 : %s\n", DigestsPerFileName, fh.attrs.filename, temp_buffer);
      }
      if (Simulate)
      {
      ...
      if (i == 32)
      {
        dbg (LOG_ALWAYS, "  __ _           _     _            ", DUMMY_ARG);
        dbg (LOG_ALWAYS, " / _| |         | |   (_)            ", DUMMY_ARG);
        dbg (LOG_ALWAYS, "| |_| | __ _ ___| |__  _ _ __   __ _ ", DUMMY_ARG);
        dbg (LOG_ALWAYS, "|  _| |/ _` / __| '_ \\| | '_ \\ / _` |", DUMMY_ARG);
        dbg (LOG_ALWAYS, "| | | | (_| \\__ \\ | | | | | | | (_| |", DUMMY_ARG);
        dbg (LOG_ALWAYS, "|_| |_|\\__,_|___/_| |_|_|_| |_|\\__, |", DUMMY_ARG);
        dbg (LOG_ALWAYS, "                                __/ |", DUMMY_ARG);
        dbg (LOG_ALWAYS, "                               |___/ ", DUMMY_ARG);
        dbg (LOG_ALWAYS, "                    _                ", DUMMY_ARG);
        dbg (LOG_ALWAYS, "                   | |               ", DUMMY_ARG);
        dbg (LOG_ALWAYS, "__      _____  _ __| | _____     ", DUMMY_ARG);
        dbg (LOG_ALWAYS, "\\ \\ /\\ / / _ \\| '__| |/ / __|        ", DUMMY_ARG);
        dbg (LOG_ALWAYS, " \\ V  V / (_) | |  |   <\\__ \\        ", DUMMY_ARG);
        ...
        if (verify_programming_sha256 == 1)
        {
          /* No file to delete mark success */
          ret = 0;
        }
        else
        {
          int result;
          /* no point in keeping readback images if flashing is success so to save space delete it */
          result = GetFullDirPath(local_temp_buffer, MAX_PATH_SIZE, MainOutputDir);
          if (result != 0)
          {
            dbg (LOG_ERROR, "Failed to get main output dir %s", MainOutputDir);
            ExitAndShowLog(1);
          }
          snprintf (local_temp_buffer+strlen(local_temp_buffer), MAX_PATH_SIZE-strlen(local_temp_buffer), "/%s", fh.attrs.filename);
          /* check if readback file exists or not before deleting it */
          if (stat(local_temp_buffer, &stat_buffer) == 0)
          {
            dbg (LOG_INFO, "Flash successful for %s, so deleting it\n", local_temp_buffer);
            ret = remove(local_temp_buffer);
          }
          else
          {
            /* do nothing if file does not exist, don't print error */
            ret = -2;
          }
        }
      }
      else
      {
        ...
        dbg (LOG_ALWAYS, "  __      _ _          _            ", DUMMY_ARG);
        dbg (LOG_ALWAYS, " / _|    (_) |        | |            ", DUMMY_ARG);
        dbg (LOG_ALWAYS, "| |_ __ _ _| | ___  __| |          ", DUMMY_ARG);
        dbg (LOG_ALWAYS, "|  _/ _` | | |/ _ \\/ _` |            ", DUMMY_ARG);
        dbg (LOG_ALWAYS, "| || (_| | | |  __/ (_| |            ", DUMMY_ARG);
      ...
      {
        dbg (LOG_ERROR, "unable to delete the read back file '%s' errno=%d", local_temp_buffer, errno);
        ExitAndShowLog (1);
      }
    ...
    */
    ...
    dbg (LOG_INFO, "", DUMMY_ARG);
    dbg (LOG_INFO, "=======================================================", DUMMY_ARG);
    dbg (LOG_INFO, "==================== {SUCCESS} ========================", DUMMY_ARG);
    dbg (LOG_INFO, "=======================================================\n\n", DUMMY_ARG);
...
// -----------------------------------------------------------------------------------------------------------------------------------------
    ...
    while (RawMode > 0)
    {
        ...
        if (ReturnTimeInSeconds (&time_start, &time_end) >= 2.0)
        {
            ...
            {
                time_throughput_calculate (&AbsoluteTimeStart, &time_end, TotalSectorsFlashed * fh.attrs.SECTOR_SIZE_IN_BYTES, -1.0, LOG_INFO );
            }
...
{
    SIZE_T FileSize = 0;
    dbg (LOG_DEBUG, "Trying get filesize, calling fseek()", DUMMY_ARG);
...
int gettimeofday (struct timeval *t, void* tzp)
{
    ...
    {
        dbg (LOG_ERROR, "clock_gettime() failed", DUMMY_ARG);
        ExitAndShowLog (1);
    }
...
void time_throughput_calculate (struct timeval *start_time, struct timeval *end_time, uint64 size_bytes, double NetworkElapsed, enum fh_log_lvl ThisLogType)
{
    ...
    char SizeString3[2048] = "";

    timeval_subtract (&result, end_time, start_time);

    if(result.tv_sec == 0 && result.tv_usec == 0)
    {
      dbg (ThisLogType, "Time measurement too small --- Overall to target 0.0 seconds (0Bps)", DUMMY_ARG);
      return;
    }
    ...
    if (NetworkElapsed > 0.0)
    {
      ...
      if (NetworkSlow < 100000000.0)
      {
        ...
        snprintf (SizeString3, sizeof(SizeString3), "FILE ACCESS SLOW!! %10s in %6.3f seconds (%10sps) --- ", SizeString2, NetworkElapsed, SizeString1);
      }
    }
...
void parseConfigXML (SIZE_T XMLFileSize, char *xml_buffer, size_t xml_buffer_size)
{
    ...
    dbg (LOG_INFO, "In parseConfigXML", DUMMY_ARG);
    ...
    while (1)
    {
        ...
        if (PacketLoc == 0)
        {
            dbg (LOG_ERROR, "2. TAG not found or recognized", DUMMY_ARG);
            return; // FIREHOSE_ERROR;
        }
...
char *deprecated_options[] = {"comportopentimeout=",
                              "createcommandtrace",
                              "createdigests",
                              "createconfigxml",
                              "dontsorttags",
...
char *hidden_options[] =
{
  "simulate",
  "skipharddriveread",
  "skipstorageinit", /* Can be deprecated, no longer needed on newer targets. */
  "skipwrite",
  "testvipimpact",
};

int is_option_in_list(char *option, char **list, size_t list_size)
{
  size_t i, len_opt, len;

  len_opt = strlen (option);
  ...
  for (i = 0; i < list_size; i++)
  {
    ...
    else if (len_opt == len - 1)
    {
      // that might have missed an equals sign here
      if (strncmp (option, list[i], len_opt) == 0 && list[i][len-1] == '=')
      {
        ShowCommandLineOptions();

        dbg (LOG_ERROR, "\n\nYou missed an '=' sign on your command line argument."
             "Look at the list of possible options."
             "\n\n\tInstead of --%s <value> use --%s=<value>\n\n\n\n", option, option);
        ExitAndShowLog (1);
      }
    }
  }
...
int PartOfCommandLineOptions (char *option)
{
  size_t size;
  int res;

  size = sizeof (CommandLineOptions) / sizeof (CommandLineOptions[0]);
  res = is_option_in_list(option, CommandLineOptions, size);
  if (res == 1)
    return 1;

  size = sizeof (deprecated_options) / sizeof (deprecated_options[0]);
  res = is_option_in_list(option, deprecated_options, size);
  if (res == 1)
    return 1;

  size = sizeof (hidden_options) / sizeof (hidden_options[0]);
  res = is_option_in_list(option, hidden_options, size);
  if (res == 1)
    return 1;
...
void ParseContentsXML (char *FileAndPath)
{
    ...
    if (FileSize == 0)
    {
        dbg (LOG_INFO, "--contentsxml file size was 0 bytes!!", DUMMY_ARG);
        ExitAndShowLog (1);
    }
    ...
    while (1)
    {
        ...
        if (ContensXMLPath[NumContentsXMLPath].Path[0] == '.')
        {
            ...
            {
              if(ContensXMLPath[NumContentsXMLPath].Path[2] == '\0' && (ContensXMLPath[NumContentsXMLPath].Path[1] == '/' || ContensXMLPath[NumContentsXMLPath].Path[1] == '\\'))
              {
              ...
              else
              {
                ...
                if ( CopyString (ContensXMLPath[NumContentsXMLPath].Path, temp_buffer, i+1, 0, len, sizeof (ContensXMLPath[NumContentsXMLPath].Path), len) == 0)
                {
                  dbg (LOG_ERROR, "Failed to copy '%s' of length %"SIZE_T_FORMAT" bytes into fh.attrs.start_sector", "0", strlen ("0") );
                  free(xml_buffer);
                  ExitAndShowLog (1);
    ...
    while (1)
    {
        ...
        if (pch != NULL)
        {
          ...
          {
            FH_LOGW("Failed to find closing </download_file>, last offset %d", CurrentPacketLoc);
            break;
          }
        }
        else
        ...
        else
        {
            ...
            {
                dbg (LOG_ERROR, "Failed to copy string into ContensXML[NumContensXML].Flavor", DUMMY_ARG);
                free(xml_buffer);
                ExitAndShowLog (1);
            }
        ...
        {
            //dbg(LOG_DEBUG,"Contents.xml is missing storage_type= and therefore assuming both eMMC and UFS", DUMMY_ARG);
            //ContensXML[NumContensXML].StorageType = 'e';  // 'e'=emmc,'u'=ufs,'n'=nand
        }
        ...
        if (pch[0] == ' ')
        {
            ...
            while (1)
            {
                ...
                if ( strncmp (ContensXML[NumContensXML].Flavor, flavor, MAX (strlen (ContensXML[NumContensXML].Flavor), strlen (flavor) ) ) != 0 )
                {
                  ...
                  {
                    FH_LOGI("Failed to match a matching flavor %s, offset %d", flavor, CurrentPacketLoc);
                    pch = pchOld;
                    break;
                  }
        ...
        }
        {
          ...
          if (ContensXML[NumContensXML].Path[len-1] != '/' && ContensXML[NumContensXML].Path[len-1] != '\\' && temp_buffer[0] != '/' && temp_buffer[0] != '\\')
          {
...
int MyCopyFile (char *FileNameSource, char *FileNameDest)
{
    ...
    if (!ForceOverwrite)
    {
      ...
      {
        dbg(LOG_INFO, "%s already exists and --forceoverwrite not set. Leaving file as is.\n", FileNameDest);
        return 1;
      }
    }
...
static firehose_error_t handleBenchmark()
{
    ...
    SIZE_T BitFlipsDetected = 0, SizeOfDataFedToHashRoutine = 0;
    char DDRCount = 0;

    dbg (LOG_INFO, "In handleBenchmark", DUMMY_ARG);
...
void ReadSha256File(void)
{
  ...
  fd = ReturnFileHandle (Sha256File, strlen (Sha256File), "rb" ); // will exit if not successful
  if(fd != NULL)
  {
    fseek(fd, sha256_file_offset * 32, SEEK_SET); //seek to the correct position in the file
    fread(verify_hash_value, 1, 32, fd);
    sha256_file_offset ++;
    fclose(fd);
    fd=NULL;
  }
